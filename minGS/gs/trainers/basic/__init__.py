...
def train(
    ...
    # Prepare model visualizer (optional — disabled by default to avoid GPU contention)
    viewer = None
    if use_viewer:
    ...
    for i in pbar:
        ...
        with torch.no_grad():
            ...
            pbar.set_description(f"Loss: {loss.item():.6f}") # We update the progress bar with the current loss.
