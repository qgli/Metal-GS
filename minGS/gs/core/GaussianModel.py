...
from metal_gs.rasterizer import MetalGaussianRasterizer, RenderSettings, simple_knn_metal
...
class GaussianModel(nn.Module):
    ...
    def forward(self, camera: BaseCamera, active_sh_degree: int=None):
        ...
        Uses Metal-GS rasterizer on Apple Silicon, with CPU↔GPU bridge.
        """
        ...
        # Extract view matrix (world_view_transform is [4,4] transposed from original)
        # Metal-GS expects row-major view matrix [4,4]
        viewmat_t = camera.world_view_transform  # already transposed in BaseCamera
        viewmat = viewmat_t.T  # undo the transpose for Metal-GS (row-major)
        campos = camera.camera_center

        W = int(camera.image_width)
        H = int(camera.image_height)
        focal_x = W / (2.0 * tan_fov_x)
        focal_y = H / (2.0 * tan_fov_y)

        # Build SH coefficients: our Metal kernel expects [N, K, 3] float16
        # minGS stores sh_coefficients as [N, num_bases, 3]
        # Only use active SH bases: (active_sh_degree + 1)^2
        num_active_bases = (active_sh_degree + 1) ** 2
        sh_coeffs = self.sh_coefficients[:, :num_active_bases, :]  # [N, K, 3]

        # Activated parameters
        opacities = self.opacity_activation(self.opacities).squeeze(-1)  # [N]
        ...
        settings = RenderSettings(
            ...
            focal_x=focal_x,
            focal_y=focal_y,
            principal_x=float(W) / 2.0,
            principal_y=float(H) / 2.0,
            img_width=W,
            img_height=H,
        ...
        # Call Metal rasterizer via autograd bridge
        rendered_image = MetalGaussianRasterizer.apply(
            self.positions,
            scales,
            rotations,
            sh_coeffs,
        ...
        # rendered_image is [H, W, 3] — convert to [3, H, W] for minGS compatibility
        rendered_image = rendered_image.permute(2, 0, 1)

        # For compatibility: set radii (used by densification stats)
        # We set all visible gaussians to have radius > 0
        # Since Metal rasterizer doesn't return per-gaussian radii easily,
        # we use a simple heuristic: all gaussians have radius 1
        self.radii = torch.ones(self.positions.shape[0], device=self.positions.device)
    ...
    def from_point_cloud(pointcloud: BasePointCloud, sh_degree: int=3, background_color: torch.Tensor=torch.tensor([0, 0, 0], dtype=torch.float32), constant_scale: float=None):
        ...
        # Initialize opacity (on CPU — will be moved to device later)
        opacities = inverse_sigmoid(0.1 * torch.ones((positions.shape[0], 1), dtype=torch.float))
    ...
    def from_ply(filename: str, sh_channels: int = 3):
        ...
        gaussian_model = GaussianModel(
            ...
            sh_coefficients=torch.cat([
                torch.tensor(sh_coefficients_0, dtype=torch.float32),
                torch.tensor(sh_coefficients_rest, dtype=torch.float32)
            ], dim=1),
